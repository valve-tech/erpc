...
func TestMain_Validate_RealConfigFile(t *testing.T) {
	...
	_, _ = f.WriteString(getWorkingConfig(8080))

	cfg, err := common.LoadConfig(fs, f.Name(), &common.DefaultOptions{})
	if err != nil {
		t.Fatalf("failed to load config: %v", err)
	}

	report := erpc.GenerateValidationReport(context.Background(), cfg)
	if report.Resources.Totals.ProjectsTotal != 1 {
		t.Fatalf("expected projectsTotal=1, got %d", report.Resources.Totals.ProjectsTotal)
	}
