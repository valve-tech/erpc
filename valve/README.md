...
| Document | What it is |
|---|---|
| [upstream-bug-log.md](upstream-bug-log.md) | **Start here.** 167 entries: 86 fixed in the fork, 63 open, 18 judged not a bug. Every entry cites `file:line` and was verified in source. Read its "Reading this file" section before you grep it — ids are sparse and out of order, and a section header is weaker evidence than an entry's own Status line. |
| [open-entry-triage.md](open-entry-triage.md) | What to do with the log's open entries, now that the fork rebases instead of sending pull requests. Ten of them obey one rule at ten different sites; fourteen were dead code and are now closed. |
